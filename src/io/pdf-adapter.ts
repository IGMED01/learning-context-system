...
import { classifyChunkKind, defaultChunkSignals, legalDocSignals, registerAdapter } from "./source-adapter.js";
...
/**
 * Split a long text into chunks of roughly maxChars,
...
async function discoverPDFs(dirPath: string): Promise<string[]> {
  const pdfs: string[] = [];
 ...
 * PDF Source Adapter.
 *
 * Scans a directory for .pdf files, extracts text using pdf-parse,
 * and produces Chunk[] compatible with LCS noise-canceler and memory store.
 *
 * Each PDF is split into page-level or paragraph-level chunks to stay
 * within token budgets. Legal/normative documents get elevated signals.
 */
export const pdfAdapter: SourceAdapter = {
  ...
  async read(sourcePath: string, options?: SourceAdapterOptions): Promise<SourceAdapterReadResult> {
    ...
    // Dynamic import to keep pdf-parse optional
    let pdfParse: (buffer: Buffer) => Promise<{ text: string; numpages: number; info: Record<string, unknown> }>;

    try {
      // pdf-parse exports vary between versions — handle both CJS and ESM
    ...
    for (const pdfPath of scanResult.items) {
      ...
      try {
        ...
        // Create an error chunk so the user knows this PDF failed
        allChunks.push({
