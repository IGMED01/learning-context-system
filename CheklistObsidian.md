...
**Versión:** v4.0 Final  
**Estado:** Implementado (cierre 2026-04-02)  
...
- [x] **1.1** Instalar dependencias: `npm install graceful-fs proper-lockfile async-mutex p-queue gray-matter`
- [x] **1.2** Extender `learning-context.config.json` con:
...
- [x] **1.3** Actualizar `src/contracts/config-contracts.ts` con validación Zod
  - Requerir credenciales solo si el backend está activo
- [x] **1.4** Actualizar `learning-context.config.production.json`

---

## Fase 2: Interface y Abstracciones

- [x] **2.1** Crear `src/integrations/knowledge-provider.ts`:
  - `name: string`
...
- [x] **2.2** Jerarquía de errores:
  - `ProviderConnectionError`
  - `ProviderWriteError`
  - `ProviderRateLimitError` (con `retryAfterMs`)
  - `ProviderValidationError`
- [x] **2.3** Decorador `withRetry(provider, policy)` con exponential backoff
- [x] **2.4** Crear `src/integrations/fs-safe.ts` — wrapper de `graceful-fs` (no monkey-patch global)
  - Exponer: `readFile`, `writeFile`, `rename`, `stat`, `unlink`, `readdir`
- [x] **2.5** Agregar tipos en `src/types/core-contracts.d.ts`

---

## Fase 3: Provider Obsidian (Fast & Thread-Safe)

- [x] **3.1** Crear `src/integrations/obsidian-provider.ts`
  - Escritura: `.md` en `NEXUS/{project}/{type}/{slug}.md`
  - Lectura: caché incremental en memoria
- [x] **3.2** Caché incremental:
...
- [x] **3.5** Formato de nota con frontmatter YAML
- [x] **3.6** Búsqueda: sobre caché en memoria, no sobre disco

---

## Fase 4: Provider Notion (Resiliente)

- [x] **4.1** Renombrar `notion-sync.js` → `notion-provider.ts`
- [x] **4.2** Implementar interface completa `KnowledgeProvider`
- [x] **4.3** Paginación: cursores para `list` y `search`
- [x] **4.4** Rate limits:
...
- [x] **4.5** Actualizar imports en archivos que referencien `notion-sync`

---

## Fase 5: Factory, DLQ y Migración

- [x] **5.1** Crear `src/integrations/knowledge-resolver.ts` (Factory Pattern)
  - Lee config → instancia provider con dependencias inyectadas
  - Cache por proyecto
  - Health check al instanciar
- [x] **5.2** Dead Letter Queue:
...
- [x] **5.3** Actualizar CLI `sync-knowledge` para usar resolver
- [x] **5.4** Nuevos CLI commands:
...
- [x] **6.1** `NEXUS/_templates/memory-entry.md`
- [x] **6.2** `NEXUS/_templates/session-close.md`
- [x] **6.3** `NEXUS/_index.md` (Map of Content)
- [x] **6.4** Estructura por proyecto: `NEXUS/{project}/{memories,axioms,sessions}/`

---

## Fase 7: Testing

- [x] **7.1** Test unitario: Caché incremental (validar que gray-matter NO se llama si mtime es igual)
- [x] **7.2** Test de concurrencia: 10 promesas de sync() al mismo `{slug}` → validar mutex
- [x] **7.3** Test de idempotencia de migración: cortar a la mitad, relanzar, validar sin duplicados
- [x] **7.4** Test de resolución de rutas: path traversal `../../` → bloqueado
- [x] **7.5** Test de resiliencia: simular caída de Notion (500/429) → validar fallback a DLQ
- [x] **7.6** Tests unitarios `ObsidianProvider`, `NotionProvider`, `KnowledgeResolver`
- [x] **7.7** Tests de integración CLI
- [x] **7.8** Test fallback `local-only` no rompe flujo existente
...
- [x] **8.2** Crear `docs/knowledge-backends.md` (guía de config + migrate + DLQ + troubleshooting)
- [x] **8.3** Actualizar `NEXUS/Bienvenido.md`
